...
const Sidebar = ({ onNewSnippet, onSelectTag, selectedTag }) => {
  ...
  return (
    <div className={`w-64 h-screen p-4 border-r ${state.theme === 'dark' ? 'bg-gray-900 text-white border-gray-700' : 'bg-white text-gray-800 border-gray-200'}`}>
...
{/* 
      {pdfContent && (
        <div className="mt-4 bg-gray-800 p-3 rounded-md">
          <p className="text-sm text-gray-400">PDF importé: {pdfContent.fileName}</p>
          <p className="text-sm text-gray-400 mb-2">Taille: {(pdfContent.fileSize / 1024).toFixed(2)} KB</p>
          <button
